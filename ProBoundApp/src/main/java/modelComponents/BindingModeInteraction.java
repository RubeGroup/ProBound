...
public class BindingModeInteraction extends ModelComponent {
	...
	public void activationAdjustment(double weight) {
		...
		//Computes the mean sequence score of this binding mode interactions
		double alphaSeq0 = b0.computeExpectedAlpha();
		double alphaSeq1 = b1.computeExpectedAlpha();
		...
		if(config.has(coefficientKey)) {
			...
			if( oSeed.has(componentKey) ) {
				...
				if(oInt.has("spacingVector")); 
					//FUTURE: Implement conversion from spacing vector to position matrix (not needed for paper).
	...
	public void regularizationToJSON_component(JSONObject out, String coefficientKey) {
		...
		if(fitComponent) {
			...
			ArrayList<ArrayList<ArrayList<double[][]>>> intReg = ModelComponent.zero_AAAdd(interactionBetas);
			
			for(int iExp=0; iExp<interactionBetas.size(); iExp++) {
				
				ArrayList<ArrayList<double[][]>> ir        = intReg.get(iExp);
				ArrayList<ArrayList<double[][]>> ib        = interactionBetas.get(iExp);
				//Computes the mean value of the non-masked coefficients.
				ArrayList<ArrayList<double[][]>> dofMatrix = ModelComponent.constant_AAdd(ib, 1);
				maskInteractionMatrix(dofMatrix);
				double nIntParam    = ModelComponent.tr_AAdd(dofMatrix);
				double betaSum      = ModelComponent.tr_AAdd(ib);
				double meanValue     = betaSum / nIntParam;
				//Subtracts the mean value from beta for non-masked coefficients.
				int nF1 = b0.maxFrames.get(iExp) / 2;
				int nF2 = b1.maxFrames.get(iExp) / 2;
				for(int s1=0; s1<2; s1++)
					for(int s2=0; s2<2; s2++)
						for(int x1=0; x1<nF1; x1++)
							for(int x2=0; x2<nF2; x2++)
								ir.get(s1).get(s2)[x1][x2] = ib.get(s1).get(s2)[x1][x2] - meanValue; 
				maskInteractionMatrix(ir);
			}
			saveToJSON_interaction_AAAdd(out, coefficientKey, intReg);

			if(fitLogActivity)
				saveToJSON_activity_Ad(out,          coefficientKey, ModelComponent.zero_Ad(activityBetas));
			else {
	...
	void readFromJSON_parameters(JSONObject in, String coefficientKey) {
		...
		activityBetas         = readFromJSON_Ad(   oInt.getJSONArray("activity"));
		interactionBetas      = readFromJSON_AAAdd(oInt.getJSONArray("positionMatrix"));
		//FUTURE: Implement functions to read "spacingVector"?
		updateAlphas();
	}
	...
	public void modifyBindingMode(JSONObject model, int iBM, int deltaLeft, int deltaRight, int deltaFlankLength, int newDIntMax, ArrayList<ModelComponent> componentList) {
		...
		//SPECIAL CASE? If a homogeneous interaction is modified, the first row/column may become zero after modifyBindingMode. The 
