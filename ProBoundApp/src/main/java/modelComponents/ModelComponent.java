...
abstract public class ModelComponent {
	...
	public String variationName;                                        // String that indicates the name of the model variation (e.g. “shift=-1”)
	abstract public Hashtable<String,JSONObject> getVariations(JSONObject in, ArrayList<ModelComponent> componentList); // Creates a list of models that should be tested.
	public boolean variationsOptimized;                                 // Indicates if the meta settings have been optimized (this is set to true when suggestVariation() returns null)
	public Hashtable<String,String> variationDescription;               // Description of variations.
	...
	public abstract void regularizationToJSON_component(JSONObject in, String coefficientKey); // Saves the variables that should be regularized to JSON object
	public abstract void indicateModelChoice(String bestKey);                           // Lets the model component respond to a choice of a specific model component.                   
	public abstract void activationAdjustment(double weight);                           // Adjusts the model component so the 'amplitude' has an appropriate magnitude. 
	...
	ModelComponent(String componentKeyIn) {
		...
		variationDescription = new Hashtable<String,String>();
		verbose              = false;
	}
	
	//Sets the internal parameters associated to with the new freezing level.
	public void setFreezingLevel(int level) {
		//System.out.println("New freezing level for "+componentName+": "+level);
		freezingLevel = level;
	...
	public void saveToJSON_settings_metadata(JSONObject out) {
		...
		oComp.put("variationsOptimized", variationsOptimized);
		oComp.put("freezingLevel",       freezingLevel);
		oComp.put("variationName",       variationName);
		if(variationDescription!=null) {
			JSONArray aVD = new JSONArray();
			oComp.put("variationDescription", aVD);
			for(String varName: variationDescription.keySet()) {
				JSONObject oVar = new JSONObject();
				aVD.put(oVar);
				oVar.put("variationName",     varName);
				oVar.put("descriptionString", variationDescription.get(varName));
			}
		}
	}
	...
	public void readFromJSON_settings_metadata(JSONObject in) {
		...
		if(oComp.has("variationsOptimized"))
			variationsOptimized = oComp.getBoolean("variationsOptimized");
		
		if(oComp.has("freezingLevel"))
			freezingLevel       = oComp.getInt("freezingLevel");
		
		if(oComp.has("variationName"))
			variationName       = oComp.getString("variationName");
		
		if(oComp.has("variationDescription")) {
			JSONArray oVD       = oComp.getJSONArray("variationDescription");
			variationDescription = new Hashtable<String,String>();
			for(int iD=0; iD <oVD.length(); iD++)
				variationDescription.put(
						oVD.getJSONObject(iD).getString("variationName"), 
						oVD.getJSONObject(iD).getString("descriptionString"));
	...
	public static JSONObject regularizationToJSON(ArrayList<ModelComponent> component, String coefficientKey) {
		...
		for(int i=0; i<component.size(); i++)
			component.get(i).regularizationToJSON_component(out, coefficientKey);
	...
	// FUNCTIONS PACKING A GRADIENT
	///////////////////////////////
	//Creates a packed gradient vector by adding the gradient components to the vector component indicated by the packing.
	public static void packGradient_O(JSONObject gradient, JSONObject packing, double[] gradVector) {
		
		Iterator<String> keys = packing.keys();

		while( keys.hasNext() ) {
			String key = (String)keys.next();
			if (      packing.get(key) instanceof JSONObject )  packGradient_O(gradient.getJSONObject(key), packing.getJSONObject(key), gradVector);
			else if ( packing.get(key) instanceof JSONArray  )  packGradient_A(gradient.getJSONArray(key),  packing.getJSONArray(key),  gradVector);
			else if ( packing.getInt(key) != -1 )               gradVector[packing.getInt(key)] = gradient.getDouble(key);
	...
	public static void packGradient_A(JSONArray gradient, JSONArray packing, double[] gradVector) {

		for(int i=0; i<packing.length(); i++) {
			if (      packing.get(i) instanceof JSONObject )  packGradient_O(gradient.getJSONObject(i), packing.getJSONObject(i), gradVector);
			else if ( packing.get(i) instanceof JSONArray  )  packGradient_A(gradient.getJSONArray(i),  packing.getJSONArray(i),  gradVector);
			else if ( packing.getInt(i) != -1 )               gradVector[packing.getInt(i)] += gradient.getDouble(i);
		}
