...
public class CombinedLikelihood extends LossFunction{
	...
	private boolean valueUpdated=false, gradientUpdated=false;
	...
	public CombinedLikelihood(JSONObject configIn) {
		...
		resetMetadataVariables();
 		...
 		/*System.out.println("================= GRADIENT AFTER SETUP =================");
		for(ModelComponent m : componentList) {
			m.setComponentInclusion(true);
			m.setFreezingLevel(0);
			m.setComponentFiting(true);
		}
		this.updatePacking();
		System.out.println("Parameters: "+Misc.formatVectorE_d(this.parameters, ",", "{", "}", 8));
		System.out.println("Packing: "+this.packing);
		ModelComponent.packModel(componentList);
		this.lossFunction_updateGradient();
		double[] vGrad  = this.gradient;
		System.out.println("Gradient:         "+Misc.formatVectorE_d(this.gradient, ",", "{", "}", 8));
		double[] vFGrad = this.gradientFiniteDifferences(0.001);
	...
	public void lossFunction_setComputeVariance(boolean use) {
		...
		super.lossFunction_setComputeVariance(use);
		
		for(CountTable c : tableModels) {
			c.computeVariance = true;
		}
	}
	...
	public void lossFunction_updateGradient() {
		...
		for(int iTable=0; iTable<tableModels.size(); iTable++) {
			...
			if(table.includeComponent) {
				try {
					tableModels.get(iTable).updateGradient();
				} catch (Exception e) {
					e.printStackTrace();
					System.out.println("> Parameters: "+Misc.formatVectorE_d(parameters));
					throw new java.lang.RuntimeException("Error while computing the gradient.");
				}
			}
		}
		...
		//Creates gradient object containing zeros for all parameters.
		JSONObject gradientObject = ModelComponent.createEmptyJSONModel("gradient");
		for(int i=0; i<componentList.size(); i++) 
			componentList.get(i).addZeroJSON_component(gradientObject, "gradient");
		
		//Creates a "gradientSquared" entry in the gradient object.
		if(computeVariance) {
			ModelComponent.addEmptyJSONObject(gradientObject, "gradientVariance");
			for(int i=0; i<componentList.size(); i++) 
				componentList.get(i).addZeroJSON_component(gradientObject, "gradientVariance");
		}
		
		//Adds gradient values to the output gradient.
		for(int i=0; i<tableModels.size(); i++) {
			CountTable table         = tableModels.get(i);
			double meanFunctionValue = table.functionValue * table.weight;
			if(table.includeComponent) {
				//Sums up the gradient and function value across count tables
				ModelComponent.addJSONObject_O(
						gradientObject.getJSONObject("gradient"), 
						table.gradient.getJSONObject("gradient"), 
						table.weight);
				value         += meanFunctionValue;
				logLikelihood += table.functionValue;
				...
				if(computeVariance) {
					...
					//Subtracts (Sum_i[gradient_i])^2/N^2
					ModelComponent.addJSONObject_O(
							gradientObject.getJSONObject("gradientVariance"), 
							ModelComponent.multiplyJSONObject_O(table.gradient.getJSONObject("gradient"), table.gradient.getJSONObject("gradient")), 
							-table.weight * table.weight);
					
					//Adds Sum_i[gradient_i^2/N]
					ModelComponent.addJSONObject_O(
							gradientObject.getJSONObject("gradientVariance"), 
							table.gradient.getJSONObject("gradientSquared"), 
							table.weight);
		...
		//Bounding term Exp[value-expBound] + Exp[-value-expBound]
		regularizationTerm += Array.sum(Array.exp(Array.add(     ModelComponent.constant_d(parameters, -expBound), parameters)))
							+ Array.sum(Array.exp(Array.subtract(ModelComponent.constant_d(parameters, -expBound), parameters)));
		...
		ModelComponent.addJSONObject_O(
				gradientObject.getJSONObject("gradient"),
				oReg.getJSONObject(          "coefficients"),
				2*lambdaL2); 
		
		//Adds dirichlet regularization to the gradient
		for(int iBM=0; iBM<bindingModes.size(); iBM++) {
			BindingMode bm = bindingModes.get(iBM);
			if(bm.includeComponent) {
				bm.addDirichletGradient(gradientObject.getJSONObject("gradient").getJSONArray("bindingModes").getJSONObject(iBM), pseudocount);
			}
		}
		
		
		//Adds the bounding term to the gradient
		JSONObject JSONState    = getJSONState().getJSONObject("coefficients");
		JSONObject JSONStateNeg = ModelComponent.scalarMultiplyJSONObject_O(JSONState, -1.0);
		ModelComponent.addJSONObject_O(
				gradientObject.getJSONObject("gradient"),
				ModelComponent.expJSONObject_O(ModelComponent.scalarAddJSONObject_O(JSONState,    -expBound)),
				1.0);
		ModelComponent.addJSONObject_O(
				gradientObject.getJSONObject("gradient"),
				ModelComponent.expJSONObject_O(ModelComponent.scalarAddJSONObject_O(JSONStateNeg, -expBound)),
				-1.0);
		
		//Packs the gradient
		gradient = new double[nParameters];
		ModelComponent.packGradient_O(gradientObject.getJSONObject("gradient"), packing.getJSONObject("packing"), gradient);
		
		//Packs the gradient variance
		if(computeVariance) {
			gradientVariance = new double[nParameters];
			ModelComponent.packGradient_O(gradientObject.getJSONObject("gradientVariance"), packing.getJSONObject("packing"), gradientVariance);
		}
	...
	public void lossFunction_nextBatch(int nDataIn) {
		//FUTURE: Implement batches when not all columns are used??
